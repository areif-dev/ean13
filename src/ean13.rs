...
impl std::error::Error for Ean13Error {}
...
impl Ean13 {
    ...
    /// ```
    pub fn from_str(input: &str) -> Result<Self, Ean13Error> {
        let normalized = match input.len() {
            // Handle UPC-A input by padding with the implied 0
            12 => format!("0{}", input),
            13 => input.to_string(),
            _ => return Err(Ean13Error::InvalidLength),
        };
        ...
        for (i, ch) in normalized.chars().enumerate() {
            digits[i] = ch
                .to_digit(10)
                .ok_or(Ean13Error::InvalidDigit)?
                .try_into()
                .or(Err(Ean13Error::InvalidDigit))?;
        }

        // Validate check digit
        let first_12: [u8; 12] = digits[0..12]
            .try_into()
            .or(Err(Ean13Error::InvalidLength))?;
    ...
    /// );
    /// ```
    pub fn from_str_nonstrict(broken_str: &str) -> Result<Ean13, Ean13Error> {
        ...
        let first_12: [u8; 12] = digit_vec.try_into().or(Err(Ean13Error::InvalidLength))?;
...
#[cfg(test)]
mod tests {
    use crate::{Ean13, Ean13Error, ean13::calculate_check_digit};
    ...
    #[test]
    fn test_from_str() {
        assert_eq!(Ean13::from_str(""), Err(Ean13Error::InvalidLength));
        assert_eq!(
            Ean13::from_str("041303015071"),
            Err(Ean13Error::InvalidCheckDigit)
        );
        assert_eq!(
        ...
        assert_eq!(
            Ean13::from_str("00413b3015071"),
            Err(Ean13Error::InvalidDigit)
        );
    }
